...
module.exports = function() {
    ...
    if (opts.webpack) {
        ...
        wpds.stdout.on('data', function(data) {
            console.log(chalk.cyan('lavabots-webpack-server: ' + data));
        });

        wpds.stderr.on('data', function(data) {
            console.error(chalk.red('lavabots-webpack-server [error]: ' + data));
        });

        wpds.on('close', function(code) {
            if (code !== 0) {
                console.log(chalk.bgRed('lavabots-webpack-server [fatal] ' + code));
            }
        });
    }
};
